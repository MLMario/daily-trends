...
**Virality score**:
The signal of how likely a Topic's Idea is to spread on a given Channel — the score each scored brief leads with. A 1–10 composite the report subagent computes per **rubric** from the Idea text + Topic context only (no source-post engagement metrics), backed by five per-dimension sub-scores and a 2-sentence justification, surfaced as a color-banded **scorebox** (red ≤4, amber 5–7, green ≥8) with per-dimension sub-score **meters** and a matching dot in the master Index. Scoring is per-Channel and **rubric-keyed**: a Channel with a rubric is scored and its section sorts by composite descending; a Channel without one (e.g. `substack`) shows an "Essay / unscored" scorebox and stays in Topic order. `linkedin` and `instagram` scoring shipped; the weight/composite/band math is the tested `scripts/lib/virality_badge.py` (the report template owns the color markup, keyed on the band).
